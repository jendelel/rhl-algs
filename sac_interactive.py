...
def parse_args(parser):
    ...
    parser.add_argument('--her_k', type=int, default=-1, help='K > 0 enables hindsight experience replay. If 0, the buffer storing is interactive. default:-1.')
    ...
    def test_agent(self, n=10):
        ...
        for test_epoch in range(n):
            ...
            while not (done or (episode_len == self.args.max_episode_len)):
                ...
                if self.render_enabled and test_epoch % self.renderSpin.value() == 0 and render:
                    self.window.render(self.env)
                    time.sleep(self.window.renderSpin.value())
                if self.feedback == FEEDBACK_FINISH:
                    self.feedback = FEEDBACK_NONE
    ...
    def train(self):
        ...
        for epoch in range(0, self.args.epochs):
            ...
            for t in range(self.args.max_episode_len):
                ...
                # Interactive HER
                if self.args.her_k == 0:
                    if obs[1] * obs2[1] < 0: # and np.abs(obs[0]) > 0.5:
                        self.feedback = FEEDBACK_STORE
                    if self.feedback == FEEDBACK_STORE:
                        self.feedback = FEEDBACK_NONE
                        print("Storing in buffer...")
                        current_state_goal = self.her_goal_f(obs2)
                        for k in range(60):
                            past_exp = np.random.randint(0, len(episode_buffer))
                            past_obs, past_action, past_reward, past_obs2, past_done = episode_buffer[past_exp]
                            her_reward, her_done = (past_reward + 100, True) if np.allclose(
                                    self.her_goal_f(past_obs2), current_state_goal) else (reward, done)
                            buffer.store(
                                    np.concatenate([past_obs, current_state_goal], axis=0), action, her_reward,
                                    np.concatenate([past_obs2, current_state_goal], axis=0), her_done)
                ...
                if done or (episode_len == self.args.max_episode_len):
                    if self.args.her_k >= 0:
                        for trans_id, (obs, action, reward, obs2, done) in enumerate(episode_buffer):
                            ...
                            # Hindsight replay buffer - future strategy.
