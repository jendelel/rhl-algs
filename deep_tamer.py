...
def parse_args(parser):
    parser.add_argument('--batch_size', type=int, default=10, help='batch_size (default: 32)')
    parser.add_argument('--learning_rate', type=float, default=0.01, help='Learning rate (default:0.01)')
    parser.add_argument('--tamer_lower_bound', type=float, default=0.2, help='TAMER Window lower bound')
    parser.add_argument('--tamer_upper_bound', type=float, default=2.0, help='TAMER Window lower bound')
...
class DeepTamer():
    ...
    def processFeedback(self, savedActions, buffer):
        ...
        new_data = [
                SavedActionWithReward(saved_action=sa, reward=self.feedback.feedback_value)
                for sa in savedActions
                if (feedback_time - sa.start_time) > self.args.tamer_lower_bound and
                (feedback_time - sa.end_time) < self.args.tamer_upper_bound
        ]
    ...
    def train(self):
        ...
        for i_episode in range(1, 10000):
            ...
            for t in range(1, 10000):  # Don't infinite loop while learning
                ...
                savedActions.append(
                        SavedAction(state=state, action_index=action, start_time=start_time, end_time=end_time))
