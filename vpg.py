...
def parse_args(parser):
    ...
    parser.add_argument('--lr_pi', type=float, default=3e-4,
                        help='Learning rate for policy optimizer. (default:3e-4)')
    parser.add_argument('--lr_V', type=float, default=1e-3,
                        help='Learning rate for value function optimizer. (default:1e-3)')
    ...
    parser.add_argument("--gae_gamma", type=float, default=0.99, 
                        help="Discount factor. (Always between 0 and 1., default: 0.99")
...
class VPG():
    def __init__(self, window, args, env):
        ...
        self.actor_critic = ActorCritic(observation_space_shape=self.env.unwrapped.observation_space.shape[0], action_space=self.env.unwrapped.action_space).to(device=self.device)
    ...
    def train(self):
        ...
        buffer = VPGBuffer(obs_dim = self.env.unwrapped.observation_space.shape, 
                            ...
                            size=self.args.steps_per_epoch, gamma=self.args.gae_gamma, lam=self.args.gae_lambda)
        
        var_counts = tuple(utils.count_parameters(module) for module in
        ...
        for epoch in range(1, self.args.epochs):
            ...
            for t in range(self.args.steps_per_epoch):
                ...
                if terminal or (t == self.args.max_episode_len -1):
                    ...
                    print(obs)
                    last_val = reward if done else self.actor_critic.value_function(torch.Tensor(obs).to(self.device).unsqueeze(dim=0)).item()
...
class CategoricalPolicyNet(nn.Module):
    def __init__(self, observation_space_shape, hidden_sizes, activation, output_activation, action_dim):
        ...
        self.logits = MLP(layers=[observation_space_shape]+list(hidden_sizes)+[action_dim], 
...
class ActorCritic(nn.Module):
    def __init__(self, observation_space_shape, action_space, hidden_sizes=[64], 
                activation=torch.tanh, output_activation=None, policy=None):
        super(ActorCritic, self).__init__()

        if policy is None and hasattr(action_space, 'n'):
            self.policy = CategoricalPolicyNet(observation_space_shape, hidden_sizes, activation, output_activation, action_dim=action_space.n)
        elif policy is None and isinstance(action_space, gym.spaces.Box):
            self.policy = GaussianPolicyNet(observation_space_shape, hidden_sizes, activation, output_activation, action_dim=action_space.shape[0])
        else:
            self.policy = policy(observation_space_shape, hidden_sizes, activation, output_activation, action_dim=action_space.shape[0])
        
        self.value_function = MLP(layers=[observation_space_shape]+list(hidden_sizes)+[1], activation=activation, output_squeeze=True)
