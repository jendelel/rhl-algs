...
class TestDeepCoachLoss(unittest.TestCase):
    ...
    def get_fake_input(self):
        ...
        def generate_window():
            ...
            return SavedActionsWithFeedback(
                    saved_actions=[gen_state_action_prob() for i in range(self.args.coach_window_size)],
    ...
    def test_loss_against_pseudocode(self):
        ...
        for saf in savedActionsWithFeedback:
            ...
            for sa in saf.saved_actions:
                ...
                e = {
                        name:
                        (self.args.eligibility_decay * e[name] + action_prob.detach() / p * param.grad.detach().clone())
                        for name, param in self.deep_coach.policy_net.named_parameters()
                }
            e_bar = {
                    name: (e_bar[name] + final_feedback * e[name])
                    for name, _ in self.deep_coach.policy_net.named_parameters()
            }
        e_bar = {
                name: (1 / (len(savedActionsWithFeedback)) * e_bar[name])
                for name, _ in self.deep_coach.policy_net.named_parameters()
        }
        self.deep_coach.optimizer.zero_grad()
        entropy.backward(retain_graph=True)
        gradients = {
                name: (e_bar[name] + self.args.entropy_reg * param.grad.detach().clone())
                for name, param in self.deep_coach.policy_net.named_parameters()
        }
        new_values = {
                name: param.data.detach().clone() + self.args.learning_rate * gradients[name]
                for name, param in self.deep_coach.policy_net.named_parameters()
        }
        ...
        for name, param in self.deep_coach.policy_net.named_parameters():
            self.assertTrue(
                    torch.allclose(param.data, new_values[name]),
                    msg="Variable: %s does not equal the value from the pseudocode!" % name)
